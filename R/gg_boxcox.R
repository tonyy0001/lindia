...
#'
#' @param fitted.lm a fitted linear model (i.e. lm, glm) that contains fitted 
#' regression
#' @param showlambda logical; controls whether lambda value should be displayed 
#' on graph. Defaults to TRUE
#' @param lambdaSF numeric; controls to how many significant figure is lambda 
...
gg_boxcox <- function(
  ...
  xend <- x[-(length(x))]
  yend <- y[-(length(y))]
  boxcox_unlist <- tibble(xstart, ystart, xend, yend)
